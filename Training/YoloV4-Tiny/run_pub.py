...
topic1 = "ps_get_sorting_result" # Photosensor
...
def subscribe(client: mqtt_client): #Photosensor 1
...
def subscribe1(client: mqtt_client): #Kecepatan Konveyor
	def on_message(client, userdata, msg):
		global speed
		print(f"Received `{msg.payload.decode()}` from `{msg.topic}` topic")
...
if __name__ == '__main__':
	...
	classesFile = "data/Tissue/side.names"
	classes = None
	...
	net = cv2.dnn.readNet(model='Side1Best.weights', config='cfg/Tissue/side.cfg')
	...
	while True:
		...
		if len(x) > 0:
			...
			if most_common == ['Flawless']:
				...
				if Wrinkled > 0:
					if Wrinkled > Shifted :
						publish(client, "Wrinkled_Samping")
					if Wrinkled > Tear :
						publish(client, "Wrinkled_Samping")
				if Shifted > 0:
					if Shifted > Wrinkled:
						publish(client, "Shifted_Samping")
					if Shifted > Tear:
						publish(client, "Shifted_Samping")
				if Tear > 0:
					if Tear > Shifted:
						publish(client, "Tear_Samping")
					if Tear > Wrinkled:
						publish(client, "Tear_Samping")
			else : 
					publish(client, "Flawless_Samping")
					# x =[]
					print("Sent to Jetson as Flawless")
			if most_common == ['Wrinkled']:
				# most_common = 'false, 1'
				publish(client, 'Wrinkled_Samping')
				# x =[]
				print("Sent to Jetson as Wrinkled")
			if most_common == ['Shifted']:
				# most_common = 'true, 3'
				publish(client, 'Shifted_Samping')
				# x =[]
				print("Sent to Jetson as Shifted")
			if most_common == ['Tear']:
				# most_common = 'true, 4'
				publish(client, 'Tear_Samping')
				# x =[]
				print("Sent to Jetson as Tear")
			if most_common == ['person']:
				# mo = 'true, 5'
				publish(client, "PERSON_HERE!")
				# print(most_common)
				# x =[]
				print("Sent to Jetson as person")
		...
		# 	print(kelas)
		# if kelas == 'Flawless':
		# 	publish(client, 'Flawless')
		# 	# print(kelas)
		# if kelas == 'Wrinkled':
		# 	publish(client, 'Wrinkled')
		# 	# print(kelas)
		# if kelas == 'Shifted':
		# 	publish(client, 'Shifted')
		# 	# print(kelas)
		# if kelas == 'Tear':
		# 	publish(client, 'Tear')
		# 	# print(kelas)
